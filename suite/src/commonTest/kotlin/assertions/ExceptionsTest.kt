...
val ExceptionAssertionsTest by preparedSuite {

	test("Should catch an exception by type") {
		checkThrows<IllegalStateException> {
			error("This should be caught by the test, and the test should be successful")
		}
	}

	test("Should not catch an exception of a different type") {
		try {
	...
	test("Should throw if no exception is thrown") {
		try {
			checkThrows<IllegalArgumentException> {
				@Suppress("unused")
				val a = 5
