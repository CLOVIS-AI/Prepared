...
To test a successful case, we use the [`failOnRaise`](../api/compat-arrow/opensavvy.prepared.compat.arrow.core/fail-on-raise.md) function:
```kotlin
...
To test a failed case, we use the [`assertRaises`](../api/compat-arrow/opensavvy.prepared.compat.arrow.core/assert-raises.md) or [`assertRaisesWith`](../api/compat-arrow/opensavvy.prepared.compat.arrow.core/assert-raises-with.md) functions:
```kotlin
test("√-1 raises") {
