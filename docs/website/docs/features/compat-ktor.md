...
!!! info "Configuration"
    ...
    See the [reference](../api/compat-ktor/index.md).
...
1.  The [`preparedServer`](../api/compat-ktor/opensavvy.prepared.compat.ktor/prepared-server.md) function declares a Ktor server as a [prepared value](prepared-values.md).
2.  Load the server module we want to test. <br/>If you use the `application.conf` file, modules are loaded automatically.
3.  The [`preparedClient`](../api/compat-ktor/opensavvy.prepared.compat.ktor/prepared-client.md) function declares a Ktor client based on a server, as a [prepared value](prepared-values.md).
