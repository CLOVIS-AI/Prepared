...
The current virtual time can be accessed using the [`time.nowMillis`](../api/suite/opensavvy.prepared.suite/-time/index.md#nowmillis) property.
...
!!! warning
    ...
    - If the service accepts a `CoroutineContext`, see [`time.scheduler`](../api/suite/opensavvy.prepared.suite/-time/index.md#scheduler).
...
See [`time.source`](../api/suite/opensavvy.prepared.suite/-time/index.md#source):
...
=== "Java Time"
    ...
    See the [reference](../api/compat-java-time/index.html).
...
=== "Java Time"
    ...
    [`time.nowJava`](../api/compat-java-time/opensavvy.prepared.compat.java.time/now-java.html) can be used to access the current time as an `Instant`:
...
=== "Java Time"
    ...
    By default, the virtual time is set to UNIX Time 0, January 1st, 1970 at 00:00:00. To set the initial time to another initial date, use [`time.set`](../api/compat-java-time/opensavvy.prepared.compat.java.time/set.md):
...
=== "Kotlin.time"
    ...
    Algorithms we want to test need to have access to the virtual time. Traditionally, this is done by accepting a `Clock` in the algorithm's constructor. We can generate such a clock with [`time.clock`](../api/suite/opensavvy.prepared.suite/-time/index.md#clock):
...
=== "Java Time"
    ...
    Algorithms we want to test need to have access to the virtual time. Traditionally, this is done by accepting a `Clock` in the algorithm's constructor. We can generate such a clock with [`time.clockJava`](../api/compat-java-time/opensavvy.prepared.compat.java.time/clock-java.md):
...
=== "Java Time"
    ...
    Instead of using `delay` to wait for a specific duration, we can also use [`delayUntil`](../api/compat-java-time/opensavvy.prepared.compat.java.time/delay-until.md) to wait until a specific instant.
